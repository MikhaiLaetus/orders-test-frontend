...
export class GlobalState {
  ...
  async initialize() {
    this.data.deliveryTypes  = (await client.query( DELIVERY_TYPES_QUERY ).toPromise()).data.deliveryTypes
    
    this.data.productStatuses = (await client.query( PRODUCT_STATUSES_QUERY ).toPromise()).data.productStatuses
    
    this.data.orderStatuses = (await client.query( ORDER_STATUSES_QUERY ).toPromise()).data.orderStatuses
  }
